    ...
    public class AddressBook1
    {
        ...
        )
        {
            try
            {
                ...
                cmd.Parameters.AddWithValue("@CompanyEn", string.IsNullOrEmpty(CompanyEn) ? dbNULL : (object)CompanyEn);
        ...
        public int AddressBook1QuickUpdate(
            string AddressBookID,
            string IsAvailable
        )
        {
            ...
            {
                var scon = new SqlConnection(connectionString);
                var cmd = new SqlCommand("usp_AddressBook1_QuickUpdate", scon);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@AddressBookID", string.IsNullOrEmpty(AddressBookID) ? dbNULL : (object)AddressBookID);
                ...
                if (errorCodeParam.Value.ToString() != "0")
                    throw new Exception("Stored Procedure 'usp_AddressBook1_QuickUpdate' reported the ErrorCode : " + errorCodeParam.Value.ToString());
