...
public partial class gout : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (((DataView)odsGout.Select()).Count <= DataPager1.PageSize)
            {
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["pci"]))
            {
                ...
                hdnlink.Value = progressTitle(dv[0]["ProductCategoryName"].ToString()) + "-pci-" + dv[0]["ProductCategoryID"].ToString() + ".aspx";
